...
public class WaterLine : MonoBehaviour
{
	...
	private void Initialize()
	{
		...
		for (int i = 0; i < size; i++)
		{
			...
			go.GetComponent<Rigidbody2D> ().constraints = RigidbodyConstraints2D.FreezePositionX | RigidbodyConstraints2D.FreezePositionY;
	...
	void Update()
	{
		...
		if (timer >= 0.5f) {
			parts [10].gameObject.transform.position = new Vector2 (transform.position.x, transform.position.y + 1f);
			Debug.Log ("XD");
			timer = 0;
		}
		#if UNITY_EDITOR
